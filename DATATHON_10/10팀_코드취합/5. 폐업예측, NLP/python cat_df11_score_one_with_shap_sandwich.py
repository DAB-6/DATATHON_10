...
CATEGORY_NAME = "sandwich"   # df11에 보통 c_sandwich 로 존재 (다르면 자동 감지 시도)
...
# =========
# c_sandwich 컬럼명 자동 탐지 (예: c_sandwich / c_sandwiches 등)
cand_cat_cols = [c for c in df.columns if c.startswith("c_") and CATEGORY_NAME in c]
if len(cand_cat_cols) == 0:
    # sandwich 관련 원핫이 없으면 경고 없이 전체에서 진행
    cat_col = None
else:
    # sandwich 키워드 포함 가장 짧은 컬럼명 선택
